...
// PostManPropWithDefensiveRanks + Goblin expansion + Unique/Duplicate slip rows (dupes capped to 10)
...
const MAX_DUPLICATE_SLIPS = Number(pm.environment.get("maxDuplicateSlips")) || 100;
...
// NEW: build ALL Goblin pairs (2-pick) with positive edge gaps
function buildAllGoblinPairs(rows) {
  ...
  const pairs = combinations2(uniq)
    .map(([a,b]) => ({ legs:[a,b], score: pairScore(a,b) + 3 })) // small boost to goblin-goblin
    .sort((x,y) => y.score - x.score);
...
// Mega slip preferring Goblins, then completing with Regulars/positives
function buildMegaSlip(rows) {
  ...
  return {
    ...
    pctWin: pWin ? pct(pWin) : "—",
    legs,
    why: "Top positive Edge Gap legs with diversity across sides/teams/periods when possible (Goblin-first)."
  };
}
...
function partitionSlipsUnique(slips) {
  ...
  // Cap duplicate list to at most MAX_DUPLICATE_SLIPS
