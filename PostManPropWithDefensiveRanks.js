// PostManPropWithDefensiveRanks + Goblin expansion + Unique/Duplicate slip rows (dupes capped to 10, no 2-Goblin 2-picks in dupes)
...
const MAX_DUPLICATE_SLIPS = Number(pm.environment.get("maxDuplicateSlips")) || 100; // default 10
...
function getBlendedStats(stats) {
  if (!(isEarlySeason)) {
    return {
      l20: nz(stats?.l20, 0), l10: nz(stats?.l10, 0), l5: nz(stats?.l5, 0), h2h: nz(stats?.h2h, 0),
...
function edgeNoteFor(outcome, rawStats, avgOdds) {
  ...
  const base = `${pick.label} ${pick.value.toFixed(2)}`;
...
function buildAllGoblinPairs(rows) {
  ...
  const pairs = combinations2(uniq)
    .map(([a, b]) => ({ legs: [a, b], score: pairScore(a, b) + 3 }))
    .sort((x, y) => y.score - x.score);
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    const pWin = pEsts.length === 2 ? product(pEsts) : null;
    out.push({
      title: `Goblin Value (2-pick) #${i + 1}`,
      size: 2,
      bucket: "Goblin",
      sortBasis: "Mix + Edge Gap",
      pctWin: pWin ? pct(pWin) : "—",
      legs: [a, b],
      why: "Two positive-edge Goblins; pairing emphasizes price + strong hit-rate windows."
    });
  }
  return out;
}
...
function buildMegaSlip(rows) {
  ...
  if (!regularPositives.length) return null;
  ...
  return {
    title: "Mega Slip (Regulars only — no duplicate names)",
    size: chosenSize,
    bucket: "Regular",
    sortBasis: "Largest positive Edge Gaps (unique players)",
    pctWin: pWin ? Math.round(pWin * 100) : "—",
    legs,
    why: "Top Regular legs by positive Edge Gap with enforced unique player names and diversity across teams/periods/sides."
  };
}
